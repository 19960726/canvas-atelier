  ...
  return (
    <section className="plan-preview" aria-label="Agent 方案预览" data-testid="plan-preview">
      ...
      <div className="plan-actions">
        <button data-testid="plan-cancel" type="button" className="plan-cancel" onClick={onCancel}><X size={14} />取消方案</button>
        <button data-testid="plan-confirm" type="button" className="plan-confirm" disabled={deletionRequired && !approvals.deleteNodes} onClick={() => onConfirm(approvals)}><Play size={14} fill="currentColor" />确认执行</button>
      </div>
    </section>
  );
}

function Approval({ dataTestId, label, ariaLabel, checked, onChange }: { dataTestId?: string; label: string; ariaLabel: string; checked: boolean; onChange: () => void }) {
