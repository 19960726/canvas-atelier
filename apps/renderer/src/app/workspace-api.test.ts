...
describe('createWorkspaceApi', () => {
  it('exposes stable UI actions without leaking desktop bridge details', async () => {
    ...
    const api = createWorkspaceApi({ addModuleNode, importDroppedMedia, flushProjectSave, saveProjectExplicitly, chatSkill, runImageGenerationNode, runReverseAgentNode, cancelModelJob, generateStoryboardNode });
    ...
    expect(Object.keys(api)).toEqual(['addModule', 'importMedia', 'save', 'chat', 'generateImage', 'reversePrompt', 'cancelJob', 'generateStoryboard']);
  });
});
