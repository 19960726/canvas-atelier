...
- 发布终态：38 文件源码提交 `5bedf5c044073e85956e8fdbf84beddbb6396031` 推送至既有分支；GitHub https://github.com/19960726/canvas-atelier/releases/tag/v1.6.174 已公开、非 prerelease、Latest。远端三项大小及 SHA-256 与本地一致：EXE 如上、blockmap 110436 bytes / `be92d6d0ba467bd315617892a6dcbed3f44342a3a02b7b80300fcaac92bb89f3`、latest.yml 375 bytes / `68ef7dc0a8ab9111cb1f32b0b450f3524bd04d0dfffe2f8780debb5cb81c3f78`；下载的 latest.yml 字节一致。发布说明明确零新增付费、42 生图模型/76 本地用例不是真实返图保证；旧客户端真实升级、外部 Photoshop/MCP 全工具、其他供应商仍未测。QA 待清目录包含证据目录，后续处理须保留报告再清测试数据，不能把目录数说成安装版本数。

### 2026-09-26 1.6.175 保存失败修复与实际项目回归（安装待窗口内容确认）

- 用户报告分层后频繁显示“本地写入失败”。根因一：成功的像素检查将 `qualityReason` 写成 `undefined`；正式 `canonicalJson` 拒绝非 JSON 值，IPC 映射为 DURABLE_WRITE_FAILED。根因二：图层重试也把旧 result/quality 字段设为 undefined。根因三：同图层重复检查沿用固定 transaction ID，新结论与旧载荷冲突。修复分别为删除已失效字段、重试时删除五个旧字段、每次新检查生成新事务 ID（原提交的传输重试仍复用原请求）。
- 之前验收漏检原因：浏览器保存替身未调用正式 canonicalJson，实际安装态只覆盖七张尺寸错误的返图；失败结论带字符串原因，未覆盖成功检查后的字段清除。新增红/绿测试在 ModuleNodeCard.test.tsx 和 app-store.test.ts，以真实序列化器检验成功状态/重试，并模拟重复事务载荷冲突。补充旧结果字段的清除和源节点不被突变检查；独立只读审查无阻塞项。
- 本次保存热修复使用 HEAD 8d11c5c 的独立源码快照 `E:/画布项目/staging-canvas-build/save-hotfix-1.6.175-r2`，只纳入上述保存修复、回归测试、Windows CRLF 测试兼容与版本号。主工作目录未完成的原图像素抠取、原位置蒙版与分层分析草稿保留改造均保留，未带入安装包；预先脏 AGENTS.md 及无关文件不提交。
- 源码验证：主目录相关 605 项通过；独立源码快照全量 4129 通过/1 失败/2 跳过，唯一失败是旧样式测试依赖 LF 换行。将测试读取规范化 CRLF 后，样式 26/26 通过；最终重试旧字段回归 app-store 266/266 通过。未把定向补测说成修复后又跑过一次全量。最终完整 npm run build 退出 0，日志在快照 work/save-hotfix-build-175-verified.log。
- 真实项目副本来自正式项目的稳定快照 revision 2599，157 节点/214 媒体资源；正式项目和活跃窗口未改动。第一次测试副本漏建 journal/archive 导致 stable-point 保存失败，此测试搭建问题已修正并重做对照。完整副本上，正式 1.6.174 的失败 IPC 明确记录 `transaction.operations.0.operation.node.data.config.qualityReason` 为 undefined；新版七层全部保存 passed/completed/version2，revision 2609，连续手动保存、正常退出及重开后图层状态/节点/资源一致、页面错误 0、退出 cleanClose=true。证据 work/formal-evidence-1.6.175/installed-before-complete-fixture.json 与 candidate-after-complete-fixture.json。图层状态通过只证明像素格式与比例检查，不证明对象位置/轮廓质量。
- 打包核验：NSIS 105294633 bytes / SHA256 d761c22f8828eaf79d132e5bc44e74dfa9680f40eb8809b78f55574782975ead；解包 86/86 文件与验收程序相同、图标和 latest.yml SHA512/大小一致；EXE 9fc23c22eb6c0cfee5430d8f509eb0d096a62c84c7c4ed1bca1432fdc0cb6122，app.asar f9d0106fa0a34d435a978ef199087a1d64a2d6556365eb1adde6dd8f32d8e63b。安装包保存在 work/formal-evidence-1.6.175。
- 层级状态：源码/构建/包内程序实际保存重开 PASS；当前正式安装目录仍 1.6.174，窗口 PID67408 仍运行。已异步询问是否存在除验证状态外的未保存编辑，不能强制关闭丢失内容。正式安装/安装后复测/公开发布尚待；新增付费请求 0、没有新的供应商成品验收。原图像素抠取和分析方案保留仍在主目录开发，旧 QA 清理审批阻挡仍未绕过。
