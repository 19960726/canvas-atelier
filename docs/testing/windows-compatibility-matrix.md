...
| Focused unit review suite | passed | `npm test -- packages/domain/src/agent-plan.test.ts packages/domain/src/project-schema.test.ts apps/renderer/src/agent/PlanPreview.test.tsx apps/renderer/src/app/app-store.test.ts apps/renderer/src/app/knowledge-client.test.ts packages/desktop-core/src/bridge-contract.test.ts packages/desktop-bridge/src/contracts.test.ts tests/integration/secret-path-scan.test.ts` -> 144 passed | Covers confirming/committing plan state, commit/queue race behavior, image_generation capability gating, production Skill candidate prepare/enrich, typed bridge contracts, and dist secret scan negative fixtures. |
| Unit/integration suite | passed | `npm test` -> 747 passed, 2 skipped | Performance-only suites remain skipped unless their explicit perf env vars are set. |
