...
| Secret/path scan | passed | `npm run scan:e2e` -> passed | Scanned `apps/renderer/src`, `packages/domain/src`, `packages/desktop-core/src`, `packages/skill-store/src`, `apps/*/dist`, `packages/*/dist`, manifests, docs/testing, tests, `playwright-report`, `test-results`, text/source maps, and trace zip text content; fixture permissions use exact evidence equality by normalized file and finding kind. |
