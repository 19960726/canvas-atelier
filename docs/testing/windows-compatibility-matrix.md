...
| --- | --- | --- | --- |
| Playwright renderer acceptance | passed | `npm run e2e` -> 7 passed | Used local Vite server, one worker, installed Edge executable, no Playwright browser download. |
...
| Focused unit review suite | passed | `npm test -- apps/renderer/src/app/app-store.test.ts packages/desktop-core/src/bridge-contract.test.ts tests/integration/secret-path-scan.test.ts` -> 108 passed; `npm test -- packages/desktop-core/src/knowledge-security.integration.test.ts apps/renderer/src/app/feedback-approval.integration.test.ts` -> 2 passed | Covers approval binding to prepared Skill preview metadata, bridge stale/concurrent write rejection, exact scan allowlist negatives, and real prepare-to-approve integration paths. |
| Unit/integration suite | passed | `npm test` -> 768 passed, 2 skipped | Performance-only suites remain skipped unless their explicit perf env vars are set. |
