...
| Focused unit review suite | passed | `npm test -- packages/domain/src/agent-plan.test.ts packages/domain/src/project-schema.test.ts apps/renderer/src/app/app-store.test.ts apps/renderer/src/agent/PlanPreview.test.tsx apps/renderer/src/canvas/CanvasWorkspace.test.tsx` -> 106 passed | Covers post-commit `waiting_for_job_retry`, retry action dedupe, no duplicate commit/undo/memory, retry UI, and project schema support. |
| Unit/integration suite | passed | `npm test` -> 753 passed, 2 skipped | Performance-only suites remain skipped unless their explicit perf env vars are set. |
