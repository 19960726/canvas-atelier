...
当前任务仍保存在 IndexedDB 中且状态为 `running`，源节点仍保存相同的 `lastResultJobId`。启动恢复不再无条件取消所有运行任务：仅当当前正式生成节点仍精确声明该任务 ID 时保留该 `running` 任务，并在项目完成 hydration 后重新启动轮询；其他排队、提交中、来源不明或已被新任务覆盖的任务仍按原规则取消。provider 再次返回已完成结果后，通过节点所有权检查补交内联结果事务。

若 provider 已无法重新返回结果，则只对当前已确认的项目、任务和资源执行一次精确修复事务；必须同时核对项目 ID、源节点 ID、`lastResultJobId`、资源 ID 和资源 SHA-256，不实现“选择最近一张未引用图片”之类的模糊自动匹配。
...
- 软件启动时只恢复仍由源节点明确拥有的 `running` 任务；其他非终态任务继续取消。
