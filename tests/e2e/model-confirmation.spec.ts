...
test('Agent conversation keeps one model conversation while switching GPT Image and Nano Banana 2', async ({ page }) => {
  ...
  await page.getByTestId('model-route-nano-banana-2').click();
  ...
  expect(state.modelSubmissions.map((submission) => submission.modelRoute)).toEqual(['nano-banana-2', 'nano-banana-2', 'gpt-image']);
  expect(state.modelSubmissions[1]?.id).toBe(state.modelSubmissions[0]?.id);
  expect(state.modelJobs.some((job) => job.status === 'running' && job.modelRoute === 'nano-banana-2')).toBe(true);
});
